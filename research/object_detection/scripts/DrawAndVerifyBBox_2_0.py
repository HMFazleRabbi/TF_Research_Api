    ...
    for f in img_list:
        ...
        try:
            data = pd.read_csv(txtfile, delimiter=",", \
                            ...
                            dtype ={'x1': int, 'y1':int, 'x2':int, 'y2':int, 'label': str})
        except:
            print("ERROR: Image: %s failed"%(f))
            print("Text: %s IMG: %s"%(outimg, txtfile))
            sys.exit()
        ...
        for index, row in data.iterrows():
            ...
            # fontScale = max (row['dy'],row['dx'])*scaleFactor
            # cv2.putText(img, row['label'], (row['x1'],row['y1']+row['dy']+yOffsetText),\
            #             font, fontScale, fontColor, lineType)
